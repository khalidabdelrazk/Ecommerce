class CategoriesResponseEntity {
  final MetadataCategoriesResponseEntity? metadata;
  final List<CategoryEntity>? data;

  CategoriesResponseEntity({this.metadata, this.data});

  factory CategoriesResponseEntity.fromJson(Map<String, dynamic> json) {
    return CategoriesResponseEntity(
      metadata: MetadataCategoriesResponseEntity.fromJson(json['metadata']),
      data: (json['data'] as List)
          .map((item) => CategoryEntity.fromJson(item))
          .toList(),
    );
  }
}

class MetadataCategoriesResponseEntity {
  final int? currentPage;
  final int? numberOfPages;
  final int? limit;

  MetadataCategoriesResponseEntity({
    this.currentPage,
    this.numberOfPages,
    this.limit,
  });

  factory MetadataCategoriesResponseEntity.fromJson(Map<String, dynamic> json) {
    return MetadataCategoriesResponseEntity(
